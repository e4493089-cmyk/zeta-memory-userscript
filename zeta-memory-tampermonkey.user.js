...
(() => {
  ...
  async function handleClick(event) {
    if (event.target.id === "zlm-settings") {
      event.target.hidden = true;
      const trigger = document.querySelector(".zlm-head-actions [data-action='toggle-settings']");
      if (trigger) trigger.textContent = "⚙ 환경설정";
      return;
    }
    ...
    if (reveal) {
      ...
      if (input) {
        ...
        reveal.setAttribute("aria-label", showing ? "값 보기" : "값 숨기기");
      }
      return;
    }
